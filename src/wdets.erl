...
%%% implemented in wdets_v8.erl, parts specific for format 9 are
%%% implemented in wdets_v9.erl.
...
%%% If a file error occurs on a working wdets file, update_mode is set
...
%%% - there is a new OS environment variable 'WDETS_DEBUG';
...
%%% by os:putenv("WDETS_DEBUG", "true")) implies that the results of
%%% calling pwrite() and pread() are tested to some extent. It also
%%% means a considerable overhead when it comes to RAM usage. The
%%% operation of Wdets is also slowed down a bit. Note that in debug
...
close(Tab) ->  
    case wdets_server:close(Tab) of
...
info(Tab) ->
    case catch wdets_server:get_pid(Tab) of
	{'EXIT', _Reason} ->
	    undefined;
	Pid ->
...
info(Tab, users) -> % undocumented
    case wdets_server:users(Tab) of
	[] ->
	    undefined;
	Users ->
	    Users
...
match(State) when State#wdets_cont.what =:= bindings ->
...
match_object(State) when State#wdets_cont.what =:= object ->
    badarg(chunk_match(State), [State]);
match_object(Term) ->
    erlang:error(badarg, [Term]).
...
open_file(File) ->
    case wdets_server:open_file(to_list(File)) of
        badarg -> % Should not happen.
            erlang:error(wdets_process_died, [File]);
        Reply -> 
            einval(Reply, [File])
    end.
...
open_file(Tab, Args) when is_list(Args) ->
    case catch defaults(Tab, Args) of
        OpenArgs when is_record(OpenArgs, open_args) ->
            case wdets_server:open_file(Tab, OpenArgs) of
                badarg -> % Should not happen.
                    erlang:error(wdets_process_died, [Tab, Args]);
                Reply -> 
...
pid2name(Pid) ->
    wdets_server:pid2name(Pid).

remove_user(Pid, From) ->
    req(Pid, {close, From}).

repair_continuation(#wdets_cont{match_program = B}=Cont, MS) 
    when is_binary(B) ->
    case ets:is_compiled_ms(B) of
	true ->
	    Cont;
	false ->
            Cont#wdets_cont{match_program = ets:match_spec_compile(MS)}
    end;
repair_continuation(#wdets_cont{}=Cont, _MS) ->
    Cont;
...
select(State) when State#wdets_cont.what =:= select ->
    badarg(chunk_match(State), [State]);
select(Term) ->
    erlang:error(badarg, [Term]).
...
istart_link(Server) ->
    {ok, proc_lib:spawn_link(wdets, init, [self(), Server])}.
...
verbose(What) ->
    ok = wdets_server:verbose(What),
    All = wdets_server:all(),
...
do_trav(#wdets_cont{bin = eof}, _Proc, Acc, _Fun) ->
    Acc;
do_trav(State, Proc, Acc, Fun) ->
...
do_trav_bins(State, Proc, Acc, Fun, [Bin | Bins]) ->
	...
	{'EXIT', _} ->
	    req(Proc, {corrupt, wdets_utils:bad_object(do_trav_bins, Bin)});
	Term ->
	    NewAcc = Fun(Term, Acc),
...
init_chunk_match(Tab, Pat, What, N) when is_integer(N), N >= 0; 
    ...
    case compile_match_spec(What, Pat) of
	{Spec, MP} ->
	    case req(wdets_server:get_pid(Tab), {match, MP, Spec, N}) of
		{done, L} ->
		    {L, #wdets_cont{tab = Tab, what = What, bin = eof}};
		{cont, State} ->
		    chunk_match(State#wdets_cont{what = What, tab = Tab});
		Error ->
		    Error
	    end;
	badarg ->
...
chunk_match(State) ->
    case catch wdets_server:get_pid(State#wdets_cont.tab) of
	{'EXIT', _Reason} ->
	    badarg;
	_Proc when State#wdets_cont.bin =:= eof ->
	    '$end_of_table';
	Proc ->
	    case req(Proc, {match_init, State}) of
		{cont, {Bins, NewState}} ->
		    MP = NewState#wdets_cont.match_program,
		    case catch do_foldl_bins(Bins, MP) of
			{'EXIT', _} ->
                            case ets:is_compiled_ms(MP) of
                                true ->
                                    Bad = wdets_utils:bad_object(chunk_match, 
                                                                Bins),
...
repl({type, T}, Defs) ->
    mem(T, [set, bag, duplicate_bag, wambo]),
...
open_file_loop(Head, N) ->
    receive 
        ...
        ?WDETS_CALL(From, {match_init, _State} = Op) ->
            do_apply_op(Op, From, Head, N);
        ?WDETS_CALL(From, {bchunk, _State} = Op) ->
            do_apply_op(Op, From, Head, N);
        ?WDETS_CALL(From, {next, _Key} = Op) ->
            do_apply_op(Op, From, Head, N);            
        ?WDETS_CALL(From, {match_delete_init, _MP, _Spec} = Op) ->
            do_apply_op(Op, From, Head, N);
    ...
    receive
        ...
        {system, From, Req} ->
            sys:handle_system_msg(Req, From, Head#head.parent, 
                                  ?MODULE, [], Head);
        Message ->
            error_logger:format("** wdets: unexpected message"
                                "(ignored): ~w~n", [Message]),
            open_file_loop(Head, N)
    end.
        ...
        _:Bad -> 
            ...
            case wdets_utils:debug_mode() of
                true ->
                    ...
                    %% the last operation.
                    error_logger:format
                      ("** wdets: Bug was found when accessing table ~w,~n"
                       "** wdets: operation was ~p and reply was ~w.~n"
                       "** wdets: Stacktrace: ~w~n",
            ...
            if
                From =/= self() ->
                    From ! {self(), {error, {wdets_bug, Name, Op, Bad}}};
...
apply_op(Op, From, Head, N) ->
	...
	auto_save ->
		...
		_Dirty when N =:= 0 -> % dirty or new_dirty
		    ...
		    wdets_utils:vformat("** wdets: Auto save of ~p\n", 
                                       [Head#head.name]), 
	...
	{insert, Objs} when Head#head.update_mode =:= dirty ->
		...
		true when Head#head.version =:= 8 ->
		    ...
		    From ! {self(), Res},
		    {N + 1, H2};
		true when Head#head.type == wambo-> 
		    wdets_wambo:insert(
		      Head#head.filename,
		      [{Key,[]}||{Key,_}<-Objs]), %EDIT
		    stream_op(Op, From, [], Head, N);
...
start_auto_save_timer(Head) ->
    ...
    erlang:send_after(Millis, self(), ?WDETS_CALL(self(), auto_save)).
...
%% Saker som ej ska ha något return value, typ insert och delete, ok/error duger
stream_end1(Pids, Next, N, C, Head, []) ->
    stream_end2(Pids, Pids, Next, N, C, Head, ok);
stream_end1(Pids, Next, N, C, Head, PwriteList) ->
    {Head1, PR} = (catch wdets_utils:pwrite(Head, PwriteList)),
    stream_end2(Pids, Pids, Next, N, C, Head1, PR).

%% Saker som behöver return values, typ lookups, member etc
...
constants(FH, FileName) ->
    ...
    if 
        ...
        Version =:= 9 ->
            wdets_v9:constants();
        true ->
            throw({error, {not_a_wdets_file, FileName}})
    end.
...
read_file_header(FileName, Access, RamFile) ->
    BF = if
		 ...
		 case file:read_file(FileName) of
		     {ok, B} -> B;
		     Err -> wdets_utils:file_error(FileName, Err)
		 end;
	     true ->
		 FileName
	 end,
    {ok, Fd} = wdets_utils:open(BF, open_args(Access, RamFile)),
    {ok, <<Version:32>>} = 
        wdets_utils:pread_close(Fd, FileName, ?FILE_FORMAT_VERSION_POS, 4),
    if 
        Version =< 8 ->
            wdets_v8:read_file_header(Fd, FileName);
        Version =:= 9 ->
            wdets_v9:read_file_header(Fd, FileName);
        true ->
            throw({error, {not_a_wdets_file, FileName}})
    end.
...
ensure_written(Head, DoSync) when Head#head.ram_file ->
    ...
    {ok, Bin} = wdets_utils:pread(Head, 0, EOF, 0),
    if
	DoSync ->
	    wdets_utils:write_file(Head, Bin);
	not DoSync ->
            case file:write_file(Head#head.filename, Bin) of
                ok -> 
                    ok;
                Error -> 
                    wdets_utils:corrupt_file(Head, Error)
            end
    end;
...
do_bchunk_init(Head, Tab) ->
	...
	{H2, []} ->
		...
		Parms ->
                    L = wdets_utils:all_allocated(H2),
                    C0 = #wdets_cont{no_objs = default, bin = <<>>, alloc = L},
		    BinParms = term_to_binary(Parms),
		    {H2, {C0#wdets_cont{tab = Tab, what = bchunk}, [BinParms]}}
	    end;
...
do_bchunk(Head, State) ->
    case wdets_v9:read_bchunks(Head, State#wdets_cont.alloc) of
	{error, Reason} ->
	    wdets_utils:corrupt_reason(Head, Reason);
	{finished, Bins} ->
	    {Head, {State#wdets_cont{bin = eof}, Bins}};
	{Bins, NewL} ->
	    {Head, {State#wdets_cont{alloc = NewL}, Bins}}
    end.
...
do_delete_all_objects(Head) ->
    ...
    ok = wdets_utils:truncate(Fd, Fname, bof),
...
ffirst(H) ->
	...
	wambo -> %EDIT
	    {NH, []} = write_cache(H),
	    {ok,R} = wdets_wambo:first(H#head.filename),
	    {NH, {ok, R}};
	_Else -> 
	    Ref = make_ref(),
...
do_safe_fixtable(Head, Pid, true) ->
	...
	false -> 
	    ...
	    Ftab = wdets_utils:get_freelists(Head),
		...
		{value, {Pid, Counter}} when Counter =:= 1; How =:= close ->
			...
			[] -> 
			    ...
			    Head#head{fixed = false, 
				  freelists = wdets_utils:get_freelists(Head)};
			NewCounters ->
			    Head#head{fixed = {TimeStamp, NewCounters}}
		    end;
		{value, {Pid, Counter}} ->
...
check_growth(Head) ->
	...
	NoThings > Head#head.next ->
	    erlang:send_after(200, self(), 
			      ?WDETS_CALL(self(), may_grow)); % Catch up.
	true ->
	    ok
    end.
...
finfo(H, delayed_write) -> % undocumented
    {H, wdets_utils:cache_size(H#head.cache)};
...
file_size(Fd, FileName) -> 
    {ok, Pos} = wdets_utils:position(Fd, FileName, eof),
    Pos.
...
test_bchunk_format(Head, Term) ->
    wdets_v9:try_bchunk_header(Term, Head) =/= not_ok.
...
do_open_file([Fname, Verbose], Parent, Server, Ref) ->
	...
	Bad ->
	    error_logger:format
	      ("** wdets: Bug was found in open_file/1, reply was ~w.~n", 
	       [Bad]),
	    {error, {wdets_bug, Fname, Bad}}
    end;
do_open_file([Tab, OpenArgs, Verb], Parent, Server, Ref) ->
	...
	Bad ->
	    error_logger:format
	      ("** wdets: Bug was found in open_file/2, arguments were~n"
	       "** wdets: ~w and reply was ~w.~n", 
	       [OpenArgs, Bad]),
	    {error, {wdets_bug, Tab, {open_file, OpenArgs}, Bad}}
    end.
...
fmatch(Head, MP, Spec, N) ->
	...
	[] -> 
		...
		{NewHead, []} ->
		    ...
		    {NewHead, {cont, C0#wdets_cont{match_program = MP}}};
		{NewHead, _} = HeadError when is_record(NewHead, head) ->
		    HeadError
	    end;
	List ->
...
fmatch_delete(Head, C) ->
	...
	{[], _} ->
	    {Head, {done, 0}};
	{RTs, NC} ->
	    MP = C#wdets_cont.match_program,
	    case catch filter_binary_terms(RTs, MP, []) of
		{'EXIT', _} ->
                    Bad = wdets_utils:bad_object(fmatch_delete, RTs),
		    wdets_utils:corrupt_reason(Head, Bad);
		Terms -> 
...
do_fmatch_delete_var_keys(Head, MP, _Spec) ->
    ...
    {NewHead, {cont, C0#wdets_cont{match_program = MP}, 0}}.
...
fnext(Head, Key) ->
    case Head#head.type of
	wambo ->
	    Next = wdets_wambo:next(Head#head.filename,Key),
	    {Head,Next};
	_ ->
...
beyond_key(_K, _Kp, []) -> [];
beyond_key(K, Kp, [H|T]) ->
    case wdets_utils:cmp(element(Kp, H), K) of
...
fopen2(Fname, Tab) ->
	...
	{ok, _} ->
		...
		{error, not_closed} ->
		    io:format(user,"wdets: file ~p not properly closed, "
			      "repairing ...~n", [Fname]),
                    Version = default,
	...
	Error ->
	    wdets_utils:file_error(Fname, Error)
    end.
...
fopen_existing_file(Tab, OpenArgs) ->
	...
	{compact, SourceHead} ->
	    io:format(user, "wdets: file ~p is now compacted ...~n", [Fname]),
		...
		_Err ->
                    _ = file:close(Fd),
                    wdets_utils:stop_disk_map(),
		    io:format(user, "wdets: compaction of file ~p failed, "
	...
	{repair, Mess} ->
	    io:format(user, "wdets: file ~p~s~n", [Fname, Mess]),
...
fopen_init_file(Tab, OpenArgs) ->
    ...
    {ok, Fd} = wdets_utils:open(FileSpec, open_args(read_write, Ram)),
    Version = if
                  UseVersion =:= default ->
                      case os:getenv("WDETS_USE_FILE_FORMAT") of
                          "8" -> 8;
                          _ -> 9
                      end;
                  true ->
...
init_disk_map(9, Name, Debug) ->
    case Debug orelse wdets_utils:debug_mode() of
        true -> 
            wdets_utils:init_disk_map(Name);
        false ->
            ok
    end;
...
version2module(V) when V =< 8 -> wdets_v8;
version2module(9) -> wdets_v9.

module2version(wdets_v8) -> 8;
module2version(wdets_v9) -> 9;
...
compact(SourceHead) ->
    ...
    Tmp = tempfile(Fname),
    TblParms = wdets_v9:table_parameters(SourceHead),
    {ok, Fd} = wdets_utils:open(Tmp, open_args(read_write, false)),
    ...
    Head = case catch wdets_v9:prep_table_copy(Fd, Tab, Tmp, Type, Kp, Ram, 
    ...
    case wdets_v9:compact_init(SourceHead, Head, TblParms) of
	{ok, NewHead} ->
	    R = case fclose(NewHead) of
		    ok ->
                        ok = file:close(SFd),
			%% Save (rename) Fname first?
			wdets_utils:rename(Tmp, Fname);
		    E ->
			E
		end,
	    if 
...
tempfile(Fname) ->
    ...
    case file:delete(Tmp) of
        {error, eacces} -> % 'wdets_process_died' happened anyway... (W-nd-ws)
            timer:sleep(5000),
            file:delete(Tmp);
        _ ->
            ok
...
fsck_copy([{_LogSz, Pos, Bins, _NoObjects} | SizeData], Head, _Bulk, NoDups)
  when is_list(Bins) ->
    ...
    wdets_utils:pwrite(Fd, FileName, PWs),
...
fsck_copy1([SzData | L], Head, Bulk, NoDups) ->
	...
	{ok, Pos} -> ok;
	PError -> wdets_utils:file_error(FileName, PError)
    end,
    {ok, Pos} = file:position(Out, Pos),
    
    io:format("copy start~n"),
    io:format("FileName:~s , Out:~w~n",[FileName,Out]),
    timer:sleep(3000),
    
    % binary gör att read operationer ger tillbaks binärer istället för listor
    % {FileName,[raw,binary]}
    CR = file:copy({FileName, [binary,read,raw]}, Out),
    
    io:format("copy end ~n"),
    timer:sleep(3000),
	...
	FError ->
	    close_files(Bulk, L, Head),
	    wdets_utils:file_error(FileName, FError)
    end;
...
free_n_objects(Head, Addr, Size, N) ->
    {NewHead, _} = wdets_utils:free(Head, Addr, Size),
    NewAddr = Addr + Size + 1,
...
do_update_counter(Head, _Key, _Incr) when (Head#head.type =/= set) or (Head#head.type =/= wambo) -> %EDIT
    {Head, badarg};
...
update_cache(Head, KeysOrObjects, What) ->
    ...
    {NewHead, ok} = wdets_utils:pwrite(Head1, PwriteList),
    {NewHead, LU}.
...
update_cache(Head, ToAdd) ->
    ...
    if 
	Lookup; NewSize >= Cache#cache.tsize -> %lookup
	...
	NewC =:= [] ->
	    {Head1, Found, []};
	Cache#cache.wrtime =:= undefined -> %insert
	    %% Empty cache. Schedule a delayed write.
	    Now = now(), Me = self(),
	    Call = ?WDETS_CALL(Me, {delayed_write, Now}),
...
cache_binary(Head, [{Q,Ks} | L], C, Seq, Ins, Lu, F) -> % lookup
    case wdets_utils:cache_lookup(Head#head.type, Ks, C, []) of
	false ->
	    cache_key_op(Head, L, C, Seq, Ins, true, F, Ks, Q);
	Found ->
	    {lookup,Pid} = Q,
...
delayed_write(Head, WrTime) ->
	...
	true ->
		...
		true ->
		    ...
		    Call = ?WDETS_CALL(Me, {delayed_write, LastWrTime}),
...
init_scan(Head, NoObjs) ->
    ...
    FreeLists = wdets_utils:get_freelists(Head),
    Base = Head#head.base,
    {From, To} = wdets_utils:find_next_allocated(FreeLists, Base, Base),
    #wdets_cont{no_objs = NoObjs, bin = <<>>, alloc = {From, To, <<>>}}.

check_safe_fixtable(Head) ->
    case (Head#head.fixed =:= false) andalso 
         ((get(verbose) =:= yes) orelse wdets_utils:debug_mode()) of
        true ->
            error_logger:format
              ("** wdets: traversal of ~p needs safe_fixtable~n",
               [Head#head.name]);
        false ->
            ok
    end.

%% -> {[RTerm], wdets_cont()} | {scan_error, Reason}
%% RTerm = {Pos, Next, Size, Status, Term}
scan(_Head, #wdets_cont{alloc = <<>>}=C) ->
    {[], C};
scan(Head, C) -> % when is_record(C, wdets_cont)
    #wdets_cont{no_objs = No, alloc = L0, bin = Bin} = C,
    {From, To, L} = L0,
    R = case No of
	    default ->
		0;
...
scan(Bin, H, From, To, L, Ts, R, {C0, Type} = C) ->
    case (H#head.mod):scan_objs(H, Bin, From, To, L, Ts, R, Type) of
        ...
        {stop, <<>>=B, NFrom, NTo, <<>>=NL, NTs} ->
            Ftab = wdets_utils:get_freelists(H),
            case wdets_utils:find_next_allocated(Ftab, NFrom, H#head.base) of
                none ->
                    {NTs, C0#wdets_cont{bin = eof, alloc = B}};
                _ -> 
                    {NTs, C0#wdets_cont{bin = B, alloc = {NFrom, NTo, NL}}}
            end;
        {stop, B, NFrom, NTo, NL, NTs} ->
            {NTs, C0#wdets_cont{bin = B, alloc = {NFrom, NTo, NL}}};
        bad_object ->
            {scan_error, wdets_utils:bad_object(scan, {From, To, Bin})}
    end.
...
scan_read(_H, From, To, _Min, L0, Ts, 
    ...
    L = {From, To, L0},
    {Ts, C#wdets_cont{bin = <<>>, alloc = L}};
scan_read(H, From, _To, Min, _L, Ts, R, C) ->
    Max = if 
	      Min < ?CHUNK_SIZE -> ?CHUNK_SIZE; 
	      true -> Min 
	  end,
    FreeLists = wdets_utils:get_freelists(H),
    case wdets_utils:find_allocated(FreeLists, From, Max, H#head.base) of
        <<>>=Bin0 ->
            {Cont, _} = C,
            {Ts, Cont#wdets_cont{bin = eof, alloc = Bin0}};
        <<From1:32,To1:32,L1/binary>> ->
            case wdets_utils:pread_n(H#head.fptr, From1, Max) of
                eof ->
		    io:format("A"),
...
err(Error) ->
    case get(verbose) of
	yes -> 
	    error_logger:format("** wdets: failed with ~w~n", [Error]),
	    Error;
	undefined  ->
	    Error
    end.
...
v_free_list(Head) ->
    io:format("FREE LIST ...... \n",[]),
    io:format("~p~n", [wdets_utils:all_free(Head)]),
