...
%% Wdets files, implementation part. This module handles versions up to
%% and including 8(c). To be called from wdets.erl only.
...
%% The version of a wdets file is indicated by the ClosedProperly
...
%% used for free lists saved on wdets files.
...
%% used by default (9, see wdets_v9.erl) does not have this problem.
...
initiate_file(Fd, Tab, Fname, Type, Kp, MinSlots, MaxSlots, 
    ...
    M = Next = ?SEGSZ * Factor,
    NoObjects = 0,
    wdets_utils:pwrite(Fd, Fname, 0, 
                      <<Freelist:32,
                      Cookie:32,
                      ClosedProperly:32,
                      (wdets_utils:type_to_code(Type)):32,
                      Version:32,
                      M:32,
                      Next:32,
                      Kp:32,
    ...
    wdets_utils:pwrite(Fd, Fname, Pos, <<SegP:32>>),
    segp_cache(Pos, SegP),
    ...
    wdets_utils:pwrite(Fd, Fname, Ws),
...
allocate_segment(Head, SegZero, SegNo) ->
    %% may throw error:
    {NewHead, Segment, _} = wdets_utils:alloc(Head, 4 * ?SEGSZ),
...
read_file_header(Fd, FileName) ->
    ...
    {ok, <<FileSize:32>>} = wdets_utils:pread_close(Fd, FileName, EOF-4, 4),
...
cache_segps(Fd, FileName, M) ->
    NSegs = no_segs(M),
    {ok, Bin} = wdets_utils:pread_close(Fd, FileName, ?HEADSZ, 4 * NSegs),
    ...
    fun(close) ->
       ...
       (L) ->
		...
		true ->
		    ...
		    {_, End, _} = wdets_utils:alloc(Head, SegSz-1),
...
output_objs2(E, Acc, Head, Cntrs, DCT, End, ChunkI, MaxNoChunks) ->
    fun(close) ->
	    ...
	    ok = wdets_utils:fwrite(Fd, FileName, 
				   wdets_utils:make_zeros(FinalZ)),
...
alloc_hole([{LSize,_} | _], Head, SegSz) when ?POW(LSize-1) > SegSz ->
    {_, SegAddr, _} = wdets_utils:alloc(Head, SegSz-1),
    Size = ?POW(LSize-1)-1,
    {_, Addr, _} = wdets_utils:alloc(Head, Size),
...
free_hole(Head, Size, N, Addr) ->
    {Head1, _} = wdets_utils:free(Head, Addr, Size),
...
allocate_all(Head, [{LSize,NoObjects} | Count], Cntrs, CT) ->
    Size = ?POW(LSize-1)-1,
    {_Head, Addr, _} = wdets_utils:alloc(Head, Size),
    NewHead = wdets_utils:alloc_many(Head, Size+1, NoObjects, Addr),
...
output_slot([], Next, Slot, CT) ->
    ...
    BinObject = if 
		    NoZeros > 100 ->
			[wdets_utils:make_zeros(NoZeros) | <<Next:32>>];
		    true ->
...
make_object(Head, Key, _LogSz, BT) when Head#head.version =:= 9 ->
    Slot = wdets_v9:db_hash(Key, Head),
    <<Slot:32, BT/binary>>;
...
do_perform_save(H) ->
    ...
    {ok, Pos} = wdets_utils:position(H, eof),
    ...
    {H2, ok} = wdets_utils:pwrite(H1, [W1,W2,W3,W4,W5]),
    {ok, Pos2} = wdets_utils:position(H2, eof),
    ?DEBUGF("Writing file size ~p, eof at ~p~n", [Pos2+4, Pos2]),
    wdets_utils:pwrite(H2, [{Pos2, <<(Pos2 + 4):32>>}]).
...
re_hash(Head, SlotStart) ->
    ...
    {ok, Bin} = wdets_utils:pread(Head, SlotPos, 4*?SEGSZ, 0),
...
re_hash_read(Head, Cs, R, RCs) ->
    {ok, Bins} = wdets_utils:pread(R, Head),
...
re_hash_read(Head, [{Pos, Size} | Ps], [C | Cs], 
	...
	BinSz when BinSz >= Sz ->
		...
		{'EXIT', _Error} ->
		    throw(wdets_utils:corrupt_reason(Head, bad_object));
		Term ->
...
re_hash_traverse_chains([], Head, Rs, Ns, Ws) ->
    {ok, Bins} = wdets_utils:pread(Rs, Head),
...
write_cache(Head) ->
	...
	true -> {Head, [], []};
	false ->
	    {NewC, _MaxInserts, PerKey} = wdets_utils:reset_cache(C),
...
chain(Head, Slot) ->
    ...
    {ok, <<Chain:32>>} = wdets_utils:pread(Head, FinalPos, 4, 0),
    {FinalPos, Chain}.
...
%% wdets:fsck/6 use for estimate has changed.
...
create_writes([{replace,Bin,Pos,OSize} | L], H, Ws, No, Next, _) ->
    Size = byte_size(Bin),
    {H1, _} = wdets_utils:free(H, Pos, OSize+?OHDSZ),
    {NH, NewPos, _} = wdets_utils:alloc(H1, ?OHDSZ + Size),
...
create_writes([{delete,Pos,Size} | L], H, Ws, No, Next, _) ->
    {NH, _} = wdets_utils:free(H, Pos, Size+?OHDSZ),
...
create_inserts(N, H, Ws, Next, Size, Bin) ->
    {NH, Pos, _} = wdets_utils:alloc(H, ?OHDSZ + Size),
    W = {Pos, [<<Next:32, Size:32, ?ACTIVE:32>>, Bin]},
...
prterm(Head, Pos, ReadAhead) ->
    Res = wdets_utils:pread(Head, Pos, ?OHDSZ, ReadAhead),
    ...
    Bin = case byte_size(Bin0) of
		  ...
		  Bin0;
	      _ ->
		  {ok, Bin1} = wdets_utils:pread(Head, Pos +  ?OHDSZ, Sz, 0),
		  Bin1
	  end,
    Term = binary_to_term(Bin),
    {Next, Sz, Term}.
...
v_segments(H, SegNo) ->
    Seg = wdets_utils:read_4(H#head.fptr, ?SEGADDR(SegNo)),
    if
	Seg =:= 0 ->
	    done;
	true ->
...
v_segment(H, SegNo, SegPos, SegSlot) ->
    ...
    Chain = wdets_utils:read_4(H#head.fptr, SegPos + (4 * SegSlot)),
    if 
