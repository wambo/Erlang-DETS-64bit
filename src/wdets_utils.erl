...
%% Utility functions common to several wdets file formats.
%% To be used from wdets, wdets_v8 and wdets_v9 only.
...
read_4(Fd, Position) ->
    ...
    <<Four:32>> = wdets_utils:read_n(Fd, 4),
    Four.
...
corrupt(Head, Error) ->
    case get(verbose) of
	yes -> 
	    error_logger:format("** wdets: Corrupt table ~p: ~p\n", 
...
%%% Very tightly coupled to the way wdets_v9 works.
%%%-----------------------------------------------------------------
...
init_disk_map(Name) ->
    error_logger:info_msg("** wdets: (debug) using disk map for ~p~n", [Name]),
